...
export default function UGCPlayer({ videoState }: { videoState: any }) {
  ...
  const [videoData, setVideoData] = useState({
    ...
    gif: "https://media.tenor.com/mOPEt9lB5aUAAAAi/drake-computer.gif", // Guaranteed transparent fallback
  ...
  useEffect(() => {
    ...
    if (rawUrl && rawUrl.includes('/render/')) {
      ...
      // Single background image generator via Pollinations AI
      ...
      // Strictly searching for transparent stickers to get clean celebrity cutouts
      fetch(`https://g.tenor.com/v1/search?q=${encodeURIComponent(cleanGifQuery + " transparent sticker")}&key=LIVDSRZULELA&limit=5`)
        .then(res => res.json())
        .then(data => {
          if (data && data.results && data.results.length > 0) {
            // Find a result that uses the ".gif" format for native transparency rendering
            const stickerUrl = data.results[0].media[0].gif.url;
    ...
    <>
      ...
      <div className="w-full flex flex-col items-center my-8" style={{ isolation: 'isolate' }}>
        <div className="relative p-[2px] rounded-[20px] bg-gradient-to-b from-blue-500/50 to-purple-600/50 shadow-[0_0_30px_rgba(139,92,246,0.2)] group hover:shadow-[0_0_40px_rgba(139,92,246,0.4)] transition-all duration-500">
          ...
          >
            {/* THE ONLY BACKGROUND LAYER (Image background as requested) */}
            <img 
              src={videoData.bg} 
              alt="" 
              className="absolute inset-0 w-full h-full object-cover opacity-80 filter brightness-95 pointer-events-none" 
            ...
            {/* CUTOUT CELEBRITY OVERLAY (Sits cleanly on top of the background) */}
            <div className="absolute inset-x-0 bottom-20 flex justify-center pointer-events-none z-[10]">
              <img 
                src={videoData.gif} 
                alt="" 
                className="w-[240px] h-auto object-contain drop-shadow-[0_10px_15px_rgba(0,0,0,0.6)]" 
        ...
        <div className="flex w-[280px] sm:w-[320px] justify-between gap-3 mt-5">
          <button onClick={() => audioRef.current && (audioRef.current.muted = !audioRef.current.muted, setIsMuted(!isMuted))} className="flex-1 bg-white/5 hover:bg-white/10 text-white/90 text-sm font-medium py-3 rounded-xl transition-all duration-300 border border-white/10 backdrop-blur-sm flex justify-center items-center gap-2">
            {isMuted ? "Unmute" : "Mute"}
          </button>
