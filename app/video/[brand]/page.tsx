...
export default function StandaloneVideoPage() {
  ...
  const bgKey = (searchParams?.get('b') || "office") as keyof typeof assetLibrary.backgrounds;
  const gifKey = (searchParams?.get('g') || "elon") as keyof typeof assetLibrary.stickers;
  ...
  const [videoData, setVideoData] = useState({
    bg: assetLibrary.backgrounds.office,
    gif: assetLibrary.stickers.elon,
    audio: assetLibrary.audio[0]
  });

  useEffect(() => {
    // Mount assets based on URL
    setVideoData({
      bg: assetLibrary.backgrounds[bgKey] || assetLibrary.backgrounds.office,
      gif: assetLibrary.stickers[gifKey] || assetLibrary.stickers.elon,
      audio: assetLibrary.audio[Math.floor(Math.random() * assetLibrary.audio.length)]
    });
    setIsLoaded(true);
  }, [bgKey, gifKey]);
  ...
  if (!isLoaded) return <div className="min-h-screen bg-black" />; // Prevent hydration mismatch
    ...
    <div className="min-h-screen bg-[#0a0a0a] flex flex-col items-center justify-center py-10 font-sans selection:bg-purple-500/30">
      ...
      <div className="relative p-[2px] rounded-[24px] bg-gradient-to-b from-blue-500/50 to-purple-600/50 shadow-[0_0_50px_rgba(139,92,246,0.2)] z-10">
        <div 
          ...
          className="relative w-[320px] h-[568px] sm:w-[360px] sm:h-[640px] bg-zinc-900 rounded-[22px] overflow-hidden cursor-pointer select-none"
        >
          ...
          <div className="absolute top-[12%] left-0 right-0 px-6 text-center pointer-events-none z-[20]">
            <h3 className="tiktok-text text-white text-[22px] sm:text-[24px] leading-[1.2] font-bold tracking-tight" style={{ wordBreak: 'break-word' }}>
              {hookText}
            </h3>
          </div>
